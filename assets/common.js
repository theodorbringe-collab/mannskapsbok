...
/* Eksempel på punkt */
export async function setItemExample(id, has_example, example_text){
  const { error } = await sb.from("items").update({ has_example, example_text }).eq("id", id);
  if(error) throw error;
}
